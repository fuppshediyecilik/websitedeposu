...
import { InsertUser, users, subscriptionPlans, userSubscriptions, payments, invoices, InsertUserSubscription, InsertPayment, InsertInvoice } from "../drizzle/schema";
...
export async function getSubscriptionPlanByStripeId(stripeId: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(subscriptionPlans).where(eq(subscriptionPlans.stripeId, stripeId)).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function updateUserSubscription(userId: number, updates: Partial<InsertUserSubscription>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updatePayment(stripePaymentIntentId: string, updates: Partial<InsertPayment>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
